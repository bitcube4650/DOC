...
SELECT *
FROM SMPBORGS
WHERE parBORGID = '304533';
-- branch
select * 
from SMPBRANCHS 
where branchid = 'BUY0103420';
...
-----------------------------------------------------------------------------------
-- 사용자 일괄 업데이트 ( 비밀번호, 전화번호, 휴대폰, 이메일)
SELECT * FROM SMPUSERS where loginid = 'ar_test';
UPDATE SMPUSERS
SET PWD		= '0cfef607fd217a3834703d1642703c3765905855766de9ed380a429e9f081f02'
,	TEL		= '4EA4719B6BFC645A23275A5D2114B453'
,	MOBILE	= '4EA4719B6BFC645A23275A5D2114B453'
,	EMAIL	= 'E7A187D3C1A5F8A54AE7A49867B363274B705F6B94D5BEB70EB9EDF3392D9008'
...
select * from SMPUSERS s where userid= '300069';
select userid, count(1) from SMPWORKINFO s group by userid; 
...
VALUES (1, '300069', '팬타온', 1, 70, 10, '01', 13, 1, 57, 'PTO', '0');

UPDATE SMPWORKINFO SET CUST_CATE_MAST_CD = 'OKP' WHERE CUST_CATE_MAST_CD IS NULL; 
-------------------------------------------------------------------------------
...
--select * from smpcodes where codetypecd = 'SMPMATERIAL' and codeval1 ='001';
...
INSERT SMP_SEQUENCE (TABLE_NAME,   NEXT_ID) VALUES ('SEQ_MCGOOD_COMP',   1);
------------------------------------------------------------------------
...
-- 업데이트 확인 필요!!!! SELECT * FROM SMP_SEQUENCE where table_name = 'SEQ_NEW_CART_ID';
---------------------------------------------------------------------------------------------
--DROP TABLE MRCART_NEW_PROD
...
( 
	...
	ADD_MASTER_YN        char(1) DEFAULT 'N'  NULL ,					-- 추가구성마스터YN
...
INSERT SMP_SEQUENCE (TABLE_NAME,   NEXT_ID) VALUES ('MCBAT_MAST',   1);
INSERT SMP_SEQUENCE (TABLE_NAME,   NEXT_ID) VALUES ('MCGOOD_BAT',   1);
...
SELECT 
	CASE
		WHEN C.repre_good = 'P' THEN B.REPRE_GOOD_IDEN_NUMB
...
--AND A.REGI_DATE_TIME BETWEEN DATEADD(MONTH, -3, GETDATE()) AND GETDATE()
GROUP BY (
	CASE
		WHEN C.repre_good = 'P' THEN B.REPRE_GOOD_IDEN_NUMB
		ELSE B.good_iden_numb 
...
SELECT * FROM SMPCODES s where AGENT_ID = '13' AND s.CODETYPECD in ( 'ORDERCHANGEMESSAGE' ) ORDER BY CODETYPEID, DISORDER;
-- CODENM2 추가
...
--select codetypecd, count(codetypecd) from SMPCODETYPES where agent_id = '13' group by codetypecd  having count(codetypecd)> 1
-- 기존 코드 업데이트 건
...
INSERT INTO SMPCODES VALUES ((select max(IIF(a.CODEID > b.CODETYPEID , a.CODEID, b.CODETYPEID))+1 from SMPCODES a, SMPCODETYPES b),(SELECT CODETYPEID FROM SMPCODETYPES WHERE CODETYPECD = 'FRONT_BG' AND AGENT_ID = '13'),N'FRONT_BG',N'팬타온 ICT 마켓',N'/assets/images/banner_panta.png',N'https://devpantaon.okplaza.kr/',N'이미지명',N'Background 이미지 src',N'바로가기 링크',0,1,N'13');
INSERT INTO SMPCODES VALUES ((select max(IIF(a.CODEID > b.CODETYPEID , a.CODEID, b.CODETYPEID))+1 from SMPCODES a, SMPCODETYPES b),(SELECT CODETYPEID FROM SMPCODETYPES WHERE CODETYPECD = 'FRONT_BG' AND AGENT_ID = '13'),N'FRONT_BG',N'SK 통신자재 오픈소싱',N'/assets/images/banner_open.png',N'/suggest',N'이미지명',N'Background 이미지 src',N'바로가기 링크',0,1,N'13');

--select * from smpcodes where codetypecd = 'FRONT_BG'
...
--SELECT * FROM SMPCODES ORDER BY CODEID DESC; SELECT * FROM SMP_SEQUENCE WHERE TABLE_NAME = 'SEQMP_CORESYSTEM';
-- 시퀸스 키 업데이트
UPDATE SMP_SEQUENCE SET NEXT_ID = (select max(IIF(a.CODEID > b.CODETYPEID , a.CODEID, b.CODETYPEID))+1 from SMPCODES a, SMPCODETYPES b) WHERE TABLE_NAME = 'SEQMP_CORESYSTEM';

-- smp_site css 컬럼 코드값으로 인서트
INSERT INTO SMP_SITE (SITE_ID, SITE_NM, SITE_DESC, CSS, LOGO_ID, RECOMM_GOODS_YN, SALE_RATE, INSERT_USER_ID, INSERT_DATE, UPDATE_USER_ID, UPDATE_DATE) VALUES ( '0', '팬타온', '팬타온 기본 설정입니다.', 0, 0,'N', 0, '2', GETDATE(), '2', GETDATE() );
INSERT INTO SMP_SITE VALUES ( '1', 'OK플라자(기본)', 'OK플라자 고객사 기본 설정입니다.', (select codeid from SMPCODES where CODETYPECD = 'FRONT_CSS_COLOR' and SMPCODES.CODEVAL1 = '기본'), 0, 'N', 0, '2', GETDATE(), '2', GETDATE() ); 
--SELECT * FROM SMP_SITE;
INSERT INTO SMP_SITE_TEMP VALUES ( '0', '팬타온', '팬타온을 대상으로 합니다.', 0, 0,'N', 0, '2', GETDATE(), '2', GETDATE(), 'Y' );
INSERT INTO SMP_SITE_TEMP VALUES ( '1', 'OK플라자(기본)', 'OK플라자 일반구매사를 대상으로 합니다.', (select codeid from SMPCODES where CODETYPECD = 'FRONT_CSS_COLOR' and SMPCODES.CODEVAL1 = '기본'), 0, 'N', 0 , '2', GETDATE(), '2', GETDATE(), 'Y' ); 

-- 메뉴명 수정
update SMPMENUS set MENUNM = '견적관리' where MENUNM = '입찰관리' and agent_id = '13' and SVCTYPECD = 'ADM' and isuse = '1';
update SMPMENUS set MENUNM = '매입실적조회' where MENUNM = '물류매입실적조회' and agent_id = '13' and SVCTYPECD = 'ADM' and isuse = '1';
update SMPMENUS set MENUNM = '신규업체선정' where MENUNM = '공급협력사 모집' and agent_id = '13' and SVCTYPECD = 'ADM' and isuse = '1';
update SMPMENUS set MENUNM = '비회원 업체관리' where MENUNM = '임시회원 업체관리' and agent_id = '13' and SVCTYPECD = 'ADM' and isuse = '1';
update SMPMENUS set MENUNM = '견적관리' where MENUNM = '입찰관리' and agent_id = '13' and SVCTYPECD = 'ADM' and isuse = '1';
update SMPMENUS set MENUNM = '상품제안현황' where MENUNM = '자재혁신제안현황' and agent_id = '13' and SVCTYPECD = 'ADM' and isuse = '1';
update SMPMENUS set MENUNM = '시스템관리' where menucd = 'ADM_SYS' and agent_id = '13' and SVCTYPECD = 'ADM' and isuse = '1';

update SMPMENUS set MENUNM = '주문/배송' where menucd = 'VEN_ORD' and agent_id = '13' and SVCTYPECD = 'VEN' and isuse = '1';
update SMPMENUS set MENUNM = '질의응답' where MENUNM = '고객의소리(Q&A)' and agent_id = '13' and SVCTYPECD = 'VEN' and isuse = '1';
update SMPMENUS set MENUNM = '신규자재 제안' where menucd = 'VEN_PDT_PROPOSAL' and agent_id = '13' and SVCTYPECD = 'VEN' and isuse = '1';
...
update SMPMENUS set isuse = '0' where MENUCD = 'ADM_PROD_CUST_CATE' and agent_id = '13' and SVCTYPECD = 'ADM';
...
-- 개발 메뉴 인서트 전에 잘못들어간 메뉴 확인
select * from SMPMENUS s where s.MENUNM like '%품종%'; --301 302 303 번 ISUSE 0 으로 할것
select * from SMPMENUS where MENUNM like '%구매사 사이트%';
...
-- 팬타온 대메뉴
INSERT INTO SMPMENUS (MENUID,MENUCD,MENUNM,TOPMENUID,PARMENUID,MENULEVEL,DISORDER,SVCTYPECD,ISFIXED,ISUSE,FWDPATH,AGENT_ID) VALUES ((select next_id +1 from SMP_SEQUENCE WHERE TABLE_NAME = 'SEQMP_CORESYSTEM'),N'ADM_PTO',N'팬타온',0,0,0,230,N'ADM',0,1,N'',N'13');
-- 팬타온 중메뉴
...
--  상품관리 > 중메뉴
INSERT INTO SMPMENUS (MENUID,MENUCD,MENUNM,TOPMENUID,PARMENUID,MENULEVEL,DISORDER,SVCTYPECD,ISFIXED,ISUSE,FWDPATH,AGENT_ID) VALUES ((select max(s.menuid) + 1 from SMPMENUS s),N'ADM_PROD_BUY_CATE',N'구매사 카테고리',(select s.MENUID from SMPMENUS s where s.MENUNM = '상품관리' and agent_id = '13' and MENULEVEL = 0 and SVCTYPECD = 'ADM'),(select s.MENUID from SMPMENUS s where s.MENUNM = '상품관리' and agent_id = '13' and MENULEVEL = 0 and SVCTYPECD = 'ADM'),1,331,N'ADM',0,1,N'/menu/product/category/buyCategoryMng.sys',N'13');
...
-- 시스템관리 > 조직관리 > 소메뉴
INSERT INTO SMPMENUS (MENUID,MENUCD,MENUNM,TOPMENUID,PARMENUID,MENULEVEL,DISORDER,SVCTYPECD,ISFIXED,ISUSE,FWDPATH,AGENT_ID) VALUES ((select max(s.menuid) + 1 from SMPMENUS s),N'ADM_WORK_MNG',N'공사유형 관리',(select s.MENUID from SMPMENUS s where s.MENUNM = '조직관리' and agent_id = '13' and MENULEVEL = 1 and SVCTYPECD = 'ADM' and isuse = '1'),(select s.MENUID from SMPMENUS s where s.MENUNM = '조직관리' and agent_id = '13' and MENULEVEL = 1 and SVCTYPECD = 'ADM' and isuse = '1'),2,954,N'ADM',0,1,N'/menu/system/borg/manageWorkTypeList.sys',N'13');
...
-- 메뉴얼 url 수정해야함
-- 중간 입력이라 시퀸스에서 가져옴
--INSERT INTO SMPMENUS (MENUID,MENUCD,MENUNM,TOPMENUID,PARMENUID,MENULEVEL,DISORDER,SVCTYPECD,ISFIXED,ISUSE,FWDPATH,AGENT_ID) VALUES ((select next_id +1 from SMP_SEQUENCE WHERE TABLE_NAME = 'SEQMP_CORESYSTEM'),N'N_BUY_MENUAL',N'메뉴얼',(select menuid from SMPMENUS where menucd = 'N_BUY_COMM'),(select menuid from SMPMENUS where menucd = 'N_BUY_COMM'),1,570,N'BUY',0,1,N'/board/faqList',N'13');
INSERT INTO SMPMENUS (MENUID,MENUCD,MENUNM,TOPMENUID,PARMENUID,MENULEVEL,DISORDER,SVCTYPECD,ISFIXED,ISUSE,FWDPATH,AGENT_ID) VALUES ((select max(s.menuid) + 1 from SMPMENUS s),N'N_BUY_MENUAL',N'메뉴얼',(select menuid from SMPMENUS where menucd = 'N_BUY_COMM'),(select menuid from SMPMENUS where menucd = 'N_BUY_COMM'),1,570,N'BUY',0,1,N'/board/faqList',N'13');
INSERT INTO SMPMENUS (MENUID,MENUCD,MENUNM,TOPMENUID,PARMENUID,MENULEVEL,DISORDER,SVCTYPECD,ISFIXED,ISUSE,FWDPATH,AGENT_ID) VALUES ((select max(s.menuid) + 1 from SMPMENUS s),N'N_BUY_SAF_MENUAL',N'메뉴얼',(select menuid from SMPMENUS where menucd = 'N_BUY_COMM'),(select menuid from SMPMENUS where menucd = 'N_BUY_COMM'),1,571,N'BUY',0,1,N'/board/faqList',N'13');
INSERT INTO SMPMENUS (MENUID,MENUCD,MENUNM,TOPMENUID,PARMENUID,MENULEVEL,DISORDER,SVCTYPECD,ISFIXED,ISUSE,FWDPATH,AGENT_ID) VALUES ((select max(s.menuid) + 1 from SMPMENUS s),N'N_BUY_HNS_MENUAL',N'메뉴얼',(select menuid from SMPMENUS where menucd = 'N_BUY_COMM'),(select menuid from SMPMENUS where menucd = 'N_BUY_COMM'),1,572,N'BUY',0,1,N'/board/faqList',N'13');
...
INSERT INTO SMPMENUS (MENUID,MENUCD,MENUNM,TOPMENUID,PARMENUID,MENULEVEL,DISORDER,SVCTYPECD,ISFIXED,ISUSE,FWDPATH,AGENT_ID) VALUES ((select max(s.menuid) + 1 from SMPMENUS s),N'VEN_CS_MENUAL',N'메뉴얼',(select menuid from smpmenus where menucd= 'VEN_CS' and agent_id = '13'),(select menuid from smpmenus where menucd= 'VEN_CS' and agent_id = '13'),1,1560,N'VEN',0,1,N'/vendor/product/vendorBidList',N'13');


-- 마지막에 시퀸스값 수정
-- SELECT * FROM SMP_SEQUENCE WHERE TABLE_NAME = 'SEQMP_CORESYSTEM';
UPDATE SMP_SEQUENCE SET NEXT_ID = (select max(s.menuid) + 1 from SMPMENUS s) WHERE TABLE_NAME = 'SEQMP_CORESYSTEM';

--select menuid from smpmenus where menucd = 'ADM_MATERIAL_SETTING'
-- 메뉴별 화면권한 추가
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_ORAGN_SITE' and isuse = 1), '1', 'ADM_ORAGN_SITE', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_ORAGN_SITE' and isuse = 1), '2', 'ADM_ORAGN_SITE', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PTO' and isuse = 1), '1', 'ADM_PTO', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PTO' and isuse = 1), '2', 'ADM_PTO', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PTO_SITE' and isuse = 1), '1', 'ADM_PTO_SITE', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PTO_SITE' and isuse = 1), '2', 'ADM_PTO_SITE', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PTO_USER' and isuse = 1), '1', 'ADM_PTO_USER', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PTO_USER' and isuse = 1), '2', 'ADM_PTO_USER', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PTO_PROJECT' and isuse = 1), '1', 'ADM_PTO_PROJECT', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PTO_PROJECT' and isuse = 1), '2', 'ADM_PTO_PROJECT', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PTO_ORDER' and isuse = 1), '1', 'ADM_PTO_ORDER', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PTO_ORDER' and isuse = 1), '2', 'ADM_PTO_ORDER', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'AMD_PTO_RTN' and isuse = 1), '1', 'AMD_PTO_RTN', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'AMD_PTO_RTN' and isuse = 1), '2', 'AMD_PTO_RTN', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PROD_BUY_CATE' and isuse = 1), '1', 'ADM_PROD_BUY_CATE', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_PROD_BUY_CATE' and isuse = 1), '2', 'ADM_PROD_BUY_CATE', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_MGT' and isuse = 1), '1', 'ADM_MATERIAL_MGT', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_MGT' and isuse = 1), '2', 'ADM_MATERIAL_MGT', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_BID' and isuse = 1), '1', 'ADM_MATERIAL_BID', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_BID' and isuse = 1), '2', 'ADM_MATERIAL_BID', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_SALE' and isuse = 1), '1', 'ADM_MATERIAL_SALE', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_SALE' and isuse = 1), '2', 'ADM_MATERIAL_SALE', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_BID' and isuse = 1), '1', 'ADM_BID', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_BID' and isuse = 1), '2', 'ADM_BID', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_SETTING' and isuse = 1), '1', 'ADM_MATERIAL_SETTING', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_SETTING' and isuse = 1), '2', 'ADM_MATERIAL_SETTING', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_VEN' and isuse = 1), '1', 'ADM_MATERIAL_VEN', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_VEN' and isuse = 1), '2', 'ADM_MATERIAL_VEN', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MCGOOD_BID' and isuse = 1), '1', 'ADM_MCGOOD_BID', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MCGOOD_BID' and isuse = 1), '2', 'ADM_MCGOOD_BID', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_COMP_BID' and isuse = 1), '1', 'ADM_COMP_BID', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_COMP_BID' and isuse = 1), '2', 'ADM_COMP_BID', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_PER_SUM' and isuse = 1), '1', 'ADM_MATERIAL_PER_SUM', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_MATERIAL_PER_SUM' and isuse = 1), '2', 'ADM_MATERIAL_PER_SUM', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_COMP_PER_SUM' and isuse = 1), '1', 'ADM_COMP_PER_SUM', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_COMP_PER_SUM' and isuse = 1), '2', 'ADM_COMP_PER_SUM', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_WORK_MNG' and isuse = 1), '1', 'ADM_WORK_MNG', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'ADM_WORK_MNG' and isuse = 1), '2', 'ADM_WORK_MNG', 'COMM_SAVE','13');
...
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'N_BUY_MENUAL'), '1', 'N_BUY_MENUAL', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'N_BUY_MENUAL'), '2', 'N_BUY_MENUAL', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'N_BUY_SAF_MENUAL'), '1', 'N_BUY_SAF_MENUAL', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'N_BUY_SAF_MENUAL'), '2', 'N_BUY_SAF_MENUAL', 'COMM_SAVE','13');

INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'N_BUY_HNS_MENUAL'), '1', 'N_BUY_HNS_MENUAL', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'N_BUY_HNS_MENUAL'), '2', 'N_BUY_HNS_MENUAL', 'COMM_SAVE','13');
...
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'VEN_CS_MENUAL'), '1', 'VEN_CS_MENUAL', 'COMM_READ','13');
INSERT INTO SMPMENUS_ACTIVITIES	(MENUID, ACTIVITYID, MENUCD, ACTIVITYCD, AGENT_ID ) VALUES ((select menuid from smpmenus where menucd = 'VEN_CS_MENUAL'), '2', 'VEN_CS_MENUAL', 'COMM_SAVE','13');
...
-- SQL Error [2601] [23000]: 고유 인덱스 'PK_SMPSCOPES_ACTIVITIES'을(를) 포함하는 개체 'dbo.SMPSCOPES_ACTIVITIES'에 중복 키 행을 삽입할 수 없습니다. 중복 키 값은 (24045, 23965, 1)입니다.

-- 영역 화면권한 연결
-- ADM_BORG_SAVE
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_BORG_SAVE' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_ORAGN_SITE'),2,N'ADM_ORAGN_SITE',N'COMM_SAVE',N'ADM_BORG_SAVE',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_BORG_SAVE' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_ORAGN_SITE'),1,N'ADM_ORAGN_SITE',N'COMM_READ',N'ADM_BORG_SAVE',N'13');

-- ADM_BORG_READ
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_BORG_READ' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_ORAGN_SITE'),1,N'ADM_ORAGN_SITE',N'COMM_READ',N'ADM_BORG_READ',N'13');

-- ADM_PRD_SAVE 상품관리영역
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_PRD_SAVE' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_PROD_BUY_CATE'),2,N'ADM_PROD_BUY_CATE',N'COMM_SAVE',N'ADM_PRD_SAVE',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_PRD_SAVE' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_PROD_BUY_CATE'),1,N'ADM_PROD_BUY_CATE',N'COMM_READ',N'ADM_PRD_SAVE',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_PRD_SAVE' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_BID'),2,N'ADM_BID',N'COMM_SAVE',N'ADM_PRD_SAVE',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_PRD_SAVE' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_BID'),1,N'ADM_BID',N'COMM_READ',N'ADM_PRD_SAVE',N'13');

-- ADM_PRD_READ 상품읽기영역
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_PRD_READ' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_PROD_BUY_CATE'),1,N'ADM_PROD_BUY_CATE',N'COMM_READ',N'ADM_PRD_READ',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_PRD_READ' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_BID'),1,N'ADM_BID',N'COMM_READ',N'ADM_PRD_READ',N'13');

-- 시스템읽기영역 ADM_SYS_READ
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_SYS_READ' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_WORK_MNG'),1,N'ADM_WORK_MNG',N'COMM_READ',N'ADM_SYS_READ',N'13');

-- 시스템관리영역 ADM_SYS_SAVE
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_SYS_SAVE' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_WORK_MNG'),2,N'ADM_WORK_MNG',N'COMM_SAVE',N'ADM_SYS_SAVE',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_SYS_SAVE' and agent_id = '13'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_WORK_MNG'),1,N'ADM_WORK_MNG',N'COMM_READ',N'ADM_SYS_SAVE',N'13');
...
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_PTO_SAVE'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_PTO'),1,N'ADM_PTO',N'COMM_READ',N'ADM_PTO_READ',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_PTO_SAVE'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'ADM_PTO'),2,N'ADM_PTO',N'COMM_READ',N'ADM_PTO_SAVE',N'13');
...
-- N_BUY_GEN	구매사 일반
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_GEN'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_MENUAL'),1,N'N_BUY_MENUAL',N'COMM_READ',N'N_BUY_GEN',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_GEN'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_MENUAL'),2,N'N_BUY_MENUAL',N'COMM_SAVE',N'N_BUY_GEN',N'13');
...
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_GEN'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_NOTICE'),1,N'N_BUY_NOTICE',N'COMM_READ',N'N_BUY_GEN',N'13');
...
-- N_BUY_CLT	구매사 관리
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_CLT'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_MENUAL'),1,N'N_BUY_MENUAL',N'COMM_READ',N'N_BUY_CLT',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_CLT'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_MENUAL'),2,N'N_BUY_MENUAL',N'COMM_SAVE',N'N_BUY_CLT',N'13');
...
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_CLT'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_NOTICE'),1,N'N_BUY_NOTICE',N'COMM_READ',N'N_BUY_CLT',N'13');
...
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_HNS_GEN'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_HNS_MENUAL'),1,N'N_BUY_HNS_MENUAL',N'COMM_READ',N'N_BUY_HNS_GEN',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_HNS_GEN'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_HNS_MENUAL'),2,N'N_BUY_HNS_MENUAL',N'COMM_SAVE',N'N_BUY_HNS_GEN',N'13');
...
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_HNS_CLT'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_HNS_MENUAL'),1,N'N_BUY_HNS_MENUAL',N'COMM_READ',N'N_BUY_HNS_CLT',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_HNS_CLT'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_HNS_MENUAL'),2,N'N_BUY_HNS_MENUAL',N'COMM_SAVE',N'N_BUY_HNS_CLT',N'13');
...
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_SAF_GEN'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_SAF_MENUAL'),1,N'N_BUY_SAF_MENUAL',N'COMM_READ',N'N_BUY_SAF_GEN',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_SAF_GEN'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_SAF_MENUAL'),2,N'N_BUY_SAF_MENUAL',N'COMM_SAVE',N'N_BUY_SAF_GEN',N'13');
...
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_SAF_CON'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_SAF_MENUAL'),1,N'N_BUY_SAF_MENUAL',N'COMM_READ',N'N_BUY_SAF_CON',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_SAF_CON'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_SAF_MENUAL'),2,N'N_BUY_SAF_MENUAL',N'COMM_SAVE',N'N_BUY_SAF_CON',N'13');
...
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_SAF_CLT'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_SAF_MENUAL'),1,N'N_BUY_SAF_MENUAL',N'COMM_READ',N'N_BUY_SAF_CLT',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_SAF_CLT'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_SAF_MENUAL'),2,N'N_BUY_SAF_MENUAL',N'COMM_SAVE',N'N_BUY_SAF_CLT',N'13');
...
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_NHS_GRP'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_HNS_MENUAL'),1,N'N_BUY_HNS_MENUAL',N'COMM_READ',N'N_BUY_NHS_GRP',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_NHS_GRP'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD =  'N_BUY_HNS_MENUAL'),2,N'N_BUY_HNS_MENUAL',N'COMM_SAVE',N'N_BUY_NHS_GRP',N'13');
...
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_SAF_SKB'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD = 'N_BUY_SAF_MENUAL'),1,N'N_BUY_SAF_MENUAL',N'COMM_READ',N'N_BUY_SAF_SKB',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'N_BUY_SAF_SKB'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD = 'N_BUY_SAF_MENUAL'),2,N'N_BUY_SAF_MENUAL',N'COMM_SAVE',N'N_BUY_SAF_SKB',N'13');
...
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'VEN_MNG2'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD = 'VEN_CS_MENUAL'),2,N'N_BUY_ORDER',N'COMM_SAVE',N'VEN_MNG2',N'13');
INSERT INTO SMPSCOPES_ACTIVITIES (SCOPEID,MENUID,ACTIVITYID,MENUCD,ACTIVITYCD,SCOPECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'VEN_MNG2'),(SELECT MENUID FROM SMPMENUS WITH(NOLOCK) WHERE MENUCD = 'VEN_CS_MENUAL'),1,N'N_BUY_ORDER',N'COMM_READ',N'VEN_MNG2',N'13');
...
-- 권한-영역 추가
INSERT INTO SMPROLES_SCOPES (SCOPEID,ROLEID,SCOPECD,ROLECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'BUY_PENTAON'),(select roleid from SMPROLES where rolecd = 'BUY_PENTAON'),(select scopeid from smpscopes where scopecd = 'BUY_PENTAON'),N'BUY_PENTAON',N'13');
...
-- 운영사 > B2B운영자권한 팬타온관리, 읽기영역 추가 (ADM_B2B_MAN)
INSERT INTO SMPROLES_SCOPES (SCOPEID,ROLEID,SCOPECD,ROLECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_PTO_SAVE'),(select roleid from smproles where rolecd = 'ADM_B2B_MAN' and agent_id = '13'),(select scopeid from smpscopes where scopecd = 'ADM_PTO_SAVE'),N'ADM_B2B_MAN',N'13');
INSERT INTO SMPROLES_SCOPES (SCOPEID,ROLEID,SCOPECD,ROLECD,AGENT_ID) VALUES ((select scopeid from smpscopes where scopecd = 'ADM_PTO_READ'),(select roleid from smproles where rolecd = 'ADM_B2B_MAN' and agent_id = '13'),(select scopeid from smpscopes where scopecd = 'ADM_PTO_READ'),N'ADM_B2B_MAN',N'13');

-----------------------------------------------------------------------

-- 그 외 기타 seq 추가
select * from SMP_SEQUENCE ss where TABLE_NAME in (
'MCNEWGOODREQUEST',
'SEQ_MCBIDAUCTION_HIST',
'SEQ_NEW_CART_PROD_ID',
'SEQ_SMPCODES',
'SMPNEW_MATERSUGGEST'
);
-- SELECT MAX(RECEIPTNUM) + 1 FROM SMPNEW_MATERSUGGEST;
-- SELECT MAX(bid_hist_id ) + 1 FROM MCBIDAUCTION_HIST;
-- select max(newgoodid) + 1 from MCNEWGOODREQUEST;
INSERT SMP_SEQUENCE (TABLE_NAME,   NEXT_ID) VALUES ('MCNEWGOODREQUEST',   (select max(newgoodid) + 1 from MCNEWGOODREQUEST));
INSERT SMP_SEQUENCE (TABLE_NAME,   NEXT_ID) VALUES ('SEQ_MCBIDAUCTION_HIST',   (SELECT MAX(bid_hist_id ) + 1 FROM MCBIDAUCTION_HIST));
INSERT SMP_SEQUENCE (TABLE_NAME,   NEXT_ID) VALUES ('SMPNEW_MATERSUGGEST',   (SELECT MAX(RECEIPTNUM) + 1 FROM SMPNEW_MATERSUGGEST));
--INSERT SMP_SEQUENCE (TABLE_NAME,   NEXT_ID) VALUES ('SEQ_NEW_CART_PROD_ID',   1); 미사용
--INSERT SMP_SEQUENCE (TABLE_NAME,   NEXT_ID) VALUES ('SEQ_SMPCODES',   1); 미사용
...
INSERT INTO SMP_TEMPLATE_MNG (TEMPLATE_CD,SEND_TYPE,SEND_SVC,TITLE,CONTENTS,DESCRIPTION,INSERT_USER_ID,INSERT_DATE,UPDATE_USER_ID,UPDATE_DATE) VALUES (N'OKPlaza097',N'KKO',N'WMS',N'물류인 경우 주문자, 인수자 발송',N'통합물류센터에서 [#{orde_iden_numb}-#{orde_sequ_numb}], [#{good_name}]을(를) 출하하였습니다. * 배송 문의 : #{admWorkInfo}',N'주문자',NULL,NULL,NULL,NULL);
...
-- 특정 카테고리 물류 상품 팬타온 업데이트 (임시 - 카테고리 확인 후 처리할 것)

-- 물류 옵션 상품 없음
INSERT MCGOODVENDOR_PANTA (
	GOOD_IDEN_NUMB
,	VENDORID
,	SALE_YN
,	SELL_PRICE
...
,		max(B.VENDORID) as VENDORID
,		'N' AS SALE_YN
,		min(B.SELL_PRICE) AS SELL_PRICE
,		min(B.SELL_PRICE) AS ORI_SELL_PRICE
,		NULL AS REPRE_GOOD_IDEN_NUMB
,		'2' AS INSERT_USER_ID
,		GETDATE() AS INSERT_DATE
,		'2' AS UPDATE_USER_ID
,		GETDATE() AS UPDATE_DATE
...
WHERE A.CATE_ID IN (
	SELECT CATE_ID
	FROM MCCATEGORYMASTER M 
	WHERE M.CATE_CD LIKE (
		SELECT CATE_CD + '%'
		FROM MCCATEGORYMASTER 
		WHERE MAJO_CODE_NAME = '소방안전'
		AND AGENT_ID = '13'
		AND CATE_LEVEL = '1'
	) AND AGENT_ID = '13'
	UNION ALL
...
JOIN (
	SELECT	
		...
		MAX(B.VENDORID) AS VENDORID
	FROM MCGOOD A
	INNER JOIN MCGOODVENDOR B ON A.GOOD_IDEN_NUMB = B.GOOD_IDEN_NUMB
	WHERE A.CATE_ID IN (
		SELECT CATE_ID
		FROM MCCATEGORYMASTER M 
		WHERE M.CATE_CD LIKE (
	...
	AND B.ISUSE = 1
	GROUP BY A.GOOD_IDEN_NUMB
) AS T ON B.GOOD_IDEN_NUMB = T.GOOD_IDEN_NUMB AND B.VENDORID = T.VENDORID;
