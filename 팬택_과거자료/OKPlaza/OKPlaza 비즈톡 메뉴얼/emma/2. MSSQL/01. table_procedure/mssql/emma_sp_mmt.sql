 ...
 *
 * MMS MT 서비스를 위한 SQLServer 2000 Stored Procedure 이다.
...
IF EXISTS (
    ...
    WHERE  name = 'sp_em_mmt_create' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_create
GO

CREATE PROCEDURE sp_em_mmt_create
AS
    ...
    BEGIN
        ...
        CREATE INDEX idx_em_mmt_tran_4 ON em_mmt_tran (emma_id)
        CREATE INDEX idx_em_mmt_tran_5 ON em_mmt_tran (msg_key)
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_log_create' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_log_create
GO

CREATE PROCEDURE sp_em_mmt_log_create
...
AS
    ...
    IF NOT EXISTS (
        ...
        BEGIN
            ...
            EXEC ('CREATE INDEX idx_em_mmt_log_'+@p_log_table+'_4 ON em_mmt_log_'+@p_log_table+' (msg_key)')
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_tran_select' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_tran_select
GO

CREATE PROCEDURE sp_em_mmt_tran_select
    @p_priority char(2)
  , @p_ttl int
  , @p_emma_id char(2)
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_client_select' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_client_select
GO

CREATE PROCEDURE sp_em_mmt_client_select
    @p_mt_pr int
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_file_select' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_file_select
GO

CREATE PROCEDURE sp_em_mmt_file_select
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_update' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_update
GO

CREATE PROCEDURE sp_em_mmt_update
    @p_table_divi             char(1)
...
AS
    ...
    IF @p_table_divi = 'M'
        SET @dsql = ' UPDATE em_mmt_tran SET  msg_key = @pp_msg_key, '
    ELSE
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_tran_rslt_update' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_tran_rslt_update
GO

CREATE PROCEDURE sp_em_mmt_tran_rslt_update
    @p_mt_report_code_ib        char(4)
  , @p_mt_report_code_ibtype    char(1)
  , @p_rs_id                    varchar(20)
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_client_rslt_update' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_client_rslt_update
GO

CREATE PROCEDURE sp_em_mmt_client_rslt_update
    @p_mt_report_code_ib        char(4)
  , @p_mt_report_code_ibtype    char(1)
  , @p_rs_id                    varchar(20)
  , @p_client_msg_key           varchar(20)
...
/****************************************************************************/
/* NAME : sp_em_mmt_tran_log_move                                           */
/* DESC : em_mmt_tran테이블의 전송 완료된 메시지를 로그 테이블로 이동한다.  */
/* PARAMETERS                                                               */
/*   IN p_emma_id : EMMA 이중화시 사용되는 EMMA ID (' ' 인 경우: 이중화 사용안함, ' ' 아닌 경우: 이중화 사용함) */
/* REMARK                                                                   */
/*   N/A                                                                    */
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_tran_log_move' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_tran_log_move
GO

CREATE PROCEDURE sp_em_mmt_tran_log_move
    @p_emma_id char(2)
AS
    ...
    SELECT TOP 2000 
         ...
         , mt_res_cnt
         , client_sub_id
    INTO   ##em_mmt_log_temp' + @v_table_suffix + '
    FROM   em_mmt_tran WITH(NOLOCK) 
    WHERE  ((msg_status = ''3''  AND input_type = ''0'') OR  (msg_status = ''4''  AND input_type = ''1''))
    AND   (broadcast_yn = ''N'' OR broadcast_yn is NULL)
    ...
    SELECT  @dsql = '
        INSERT em_mmt_log_'+@v_log_table+' 
               SELECT temp.mt_pr
					...
					, temp.mt_res_cnt
					, temp.client_sub_id
               FROM   ##em_mmt_log_temp' + @v_table_suffix + ' temp WITH(NOLOCK) 
               WHERE NOT EXISTS( SELECT A.mt_pr FROM em_mmt_log_'+@v_log_table+'  A WHERE A.mt_pr = temp.mt_pr) '
	...
	SELECT @dsql = '
    DELETE em_mmt_tran FROM em_mmt_tran A, ##em_mmt_log_temp' + @v_table_suffix + ' B
	...
	SELECT @dsql = '
    DROP TABLE ##em_mmt_log_temp' + @v_table_suffix
...
/****************************************************************************/
/* NAME : sp_em_mmt_client_log_move                                         */
/* DESC : em_mmt_client테이블의 전송 완료된 메시지를 로그 테이블로 이동한다. */
/* PARAMETERS                                                               */
/*   IN p_emma_id : EMMA 이중화시 사용되는 EMMA ID (' ' 인 경우: 이중화 사용안함, ' ' 아닌 경우: 이중화 사용함) */
/* REMARK                                                                   */
/*   N/A                                                                    */
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_client_log_move' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_client_log_move
GO

CREATE PROCEDURE sp_em_mmt_client_log_move
    @p_emma_id char(2)
AS
    ...
    SELECT TOP 1000 
         ...
         , A.client_sub_id         AS client_sub_id
    INTO   ##em_mmt_log_temp' + @v_table_suffix + '
    FROM   em_mmt_tran A WITH(NOLOCK)
       ,   em_mmt_client B WITH(NOLOCK)
    WHERE  A.mt_pr = B.mt_pr
    ...
    SELECT  @dsql = '
        INSERT em_mmt_log_'+@v_log_table+' 
               SELECT temp.mt_pr
					...
					, temp.client_sub_id
               FROM   ##em_mmt_log_temp' + @v_table_suffix + ' temp WITH(NOLOCK) 
               WHERE NOT EXISTS( SELECT A.mt_pr FROM em_mmt_log_'+@v_log_table+'  A WHERE A.mt_pr = temp.mt_pr AND A.mt_seq = temp.mt_seq) '
	...
	SELECT @dsql = '
    DELETE em_mmt_client
    FROM   em_mmt_client A
       ,   ##em_mmt_log_temp' + @v_table_suffix + ' B
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_tran_rslt_delete' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_tran_rslt_delete
GO

CREATE PROCEDURE sp_em_mmt_tran_rslt_delete
AS
...
/* DESC : em_mmt_tran테이블의 유효기간이 지난 메시지를 로그 테이블로 이동한다.  */
/* PARAMETERS                                                               */
/*   IN p_emma_id : EMMA 이중화시 사용되는 EMMA ID (' ' 인 경우: 이중화 사용안함, ' ' 아닌 경우: 이중화 사용함) */
/* REMARK                                                                   */
/*   N/A                                                                    */
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_tran_log_move_past' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_tran_log_move_past
GO

CREATE PROCEDURE sp_em_mmt_tran_log_move_past
    @p_emma_id char(2)
AS
    ...
    SELECT TOP 50000 
         ...
         , mt_res_cnt
         , client_sub_id
    INTO   ##em_mmt_log_temp_past' + @v_table_suffix + '
    FROM   em_mmt_tran WITH(NOLOCK) 
    ...
    SELECT  @dsql = '
        INSERT em_mmt_log_'+@v_log_table+' 
               SELECT temp.mt_pr
					...
					, temp.mt_res_cnt
					, temp.client_sub_id
               FROM   ##em_mmt_log_temp_past' + @v_table_suffix + ' temp WITH(NOLOCK) 
               WHERE NOT EXISTS( SELECT A.mt_pr FROM em_mmt_log_'+@v_log_table+'  A WHERE A.mt_pr = temp.mt_pr) '
	...
	SELECT @dsql = '
    DELETE em_mmt_tran FROM em_mmt_tran A, ##em_mmt_log_temp_past' + @v_table_suffix + ' B
    WHERE A.mt_pr = B.mt_pr'
	...
	SELECT @dsql = '
    DROP TABLE ##em_mmt_log_temp_past' + @v_table_suffix
    EXEC sp_executesql @dsql
...
/* DESC : em_mmt_client테이블의 유효기간이 지난 메시지를 로그 테이블로 이동한다.    */
/* PARAMETERS                                                               */
/*   IN p_emma_id : EMMA 이중화시 사용되는 EMMA ID (' ' 인 경우: 이중화 사용안함, ' ' 아닌 경우: 이중화 사용함) */
/* REMARK                                                                   */
/*   N/A                                                                    */
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmt_client_log_move_past' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmt_client_log_move_past
GO

CREATE PROCEDURE sp_em_mmt_client_log_move_past
    @p_emma_id char(2)
AS
    ...
    DELETE em_mmt_client
    FROM   em_mmt_client A
         , ##em_mmt_log_temp_past' + @v_table_suffix + ' B
    WHERE  A.mt_pr = ' + CAST(@v_mt_pr as varchar) + '
    AND    A.mt_pr = B.mt_pr
    AND    A.mt_seq = B.mt_seq'
