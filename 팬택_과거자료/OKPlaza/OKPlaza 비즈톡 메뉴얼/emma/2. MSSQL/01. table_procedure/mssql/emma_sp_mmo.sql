 ...
 *
 * MMS MO 서비스를 위한 SQLServer 2000 Stored Procedure 이다.
...
/****************************************************************************/
/* NAME : sp_em_mmo_create                                                  */
/* DESC : MMS MO 서비스 관련 테이블을 생성한다.                             */
/* PARAMETERS                                                               */
/*   N/A                                                                    */
/* REMARK                                                                   */
/*   em_mo_tran :  MMSMO, 수신 테이블                                       */
/*   em_mmo_file : 첨부파일 정보 테이블                                     */
/*   SMSMO, MMSMO 는 em_mo_tran, em_mo_log_yyyymm을 같이 사용한다.          */
/****************************************************************************/
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmo_create' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmo_create
GO

CREATE PROCEDURE sp_em_mmo_create
AS

    IF NOT EXISTS (
        SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'em_mo_tran'
    )
    BEGIN
    ...
    IF NOT EXISTS (
        SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'em_mmo_file'
    )
    BEGIN
        CREATE TABLE em_mmo_file
        ( mo_key              varchar(50)  COLLATE Korean_Wansung_CS_AS NOT NULL
        , mo_seq              int NOT NULL
        , attach_file_key     varchar(80)  COLLATE Korean_Wansung_CS_AS NOT NULL
        , attach_file_subpath varchar(64)  COLLATE Korean_Wansung_CS_AS NOT NULL
        , attach_file_name    varchar(255) COLLATE Korean_Wansung_CS_AS NOT NULL
        , attach_file_size    int NOT NULL
        , attach_file_type    varchar(20)  COLLATE Korean_Wansung_CS_AS NOT NULL
        , reg_date            datetime NOT NULL default getdate()
        )
        
        ALTER TABLE em_mmo_file  ADD PRIMARY KEY(mo_key, mo_seq)
        
        CREATE INDEX idx_em_mmo_file_1 ON em_mmo_file(attach_file_name)
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmo_log_create' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmo_log_create
GO

CREATE PROCEDURE sp_em_mmo_log_create
...
AS
    ...
    IF NOT EXISTS (
        SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'em_mo_log_'+@p_log_table
    )
    BEGIN
        select @dsql = '
    ...
    IF NOT EXISTS (
        SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'em_mmo_file_log_'+@p_log_table
    )
    ...
    BEGIN
        select @dsql = '
            ...
            EXEC ('ALTER TABLE em_mmo_file_log_'+@p_log_table+' ADD PRIMARY KEY ( mo_key, mo_seq )')
            EXEC ('CREATE INDEX idx_em_mmo_file_log_'+@p_log_table+'_1 ON em_mmo_file_log_'+@p_log_table+' (attach_file_name)')
...
/*   IN p_carrier        : 착신망 정보 10001(SKT), 10002(KTF), 10003(LGT)                                              */
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmo_tran_insert' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmo_tran_insert
GO

CREATE PROCEDURE sp_em_mmo_tran_insert
    @p_rs_id          VARCHAR(20)
...
AS
    ...
    DECLARE @dsql nvarchar(2000)
          , @v_param  nvarchar(500)
          , @v_log_table varchar(6)
          , @v_mokey varchar(50)

    SELECT @v_log_table = convert(varchar, @p_date_mo, 112)

    /* log table check */
    EXEC sp_em_mmo_log_create @v_log_table
    ...
    SELECT @dsql = ' INSERT INTO em_mo_log_' + @v_log_table + ' ( '
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmo_file_insert' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmo_file_insert
GO

CREATE PROCEDURE sp_em_mmo_file_insert
    @p_date_mo             DATETIME
  , @p_client_msg_key      VARCHAR(50)
  , @p_mo_seq              INT
  , @p_attach_file_key     VARCHAR(80)
  , @p_attach_file_subpath VARCHAR(64)
  , @p_attach_file_name    VARCHAR(255)
  , @p_attach_file_size    INT
  , @p_attach_file_type    VARCHAR(20)
AS

    DECLARE @dsql        nvarchar(1000)
          , @v_param     nvarchar(300)
          , @v_log_table varchar(6)
          , @v_mokey     varchar(50)
    ...
    SELECT @dsql = 'SELECT @pp_mokey = mo_key FROM em_mmo_file_log_' + @v_log_table + ' WHERE mo_key = @pp_client_msg_key and mo_seq = @pp_mo_seq';
    SET @v_param = '  @pp_mokey VARCHAR(50) OUTPUT
                    , @pp_client_msg_key VARCHAR(50)
					, @pp_mo_seq INT'
    ...
    SET @dsql = ' INSERT INTO em_mmo_file_log_' + @v_log_table + ' ( '
    ...
    SET @v_param = '
        @pp_client_msg_key VARCHAR(50)
      , @pp_mo_seq INT
      , @pp_attach_file_key VARCHAR(80)
      , @pp_attach_file_subpath VARCHAR(64)
      , @pp_attach_file_name VARCHAR(255)
      , @pp_attach_file_size INT
      , @pp_attach_file_type VARCHAR(20) '

    EXECUTE sp_executesql @dsql, @v_param
                        , @pp_client_msg_key      = @p_client_msg_key
                        , @pp_mo_seq              = @p_mo_seq
                        , @pp_attach_file_key     = @p_attach_file_key
                        , @pp_attach_file_subpath = @p_attach_file_subpath
                        , @pp_attach_file_name    = @p_attach_file_name
                        , @pp_attach_file_size    = @p_attach_file_size
                        , @pp_attach_file_type    = @p_attach_file_type
...
/****************************************************************************/
/* NAME : sp_em_mmo_tran_delete                                             */
/* DESC : 수신된 MMS MO Packet을 MMO 테이블에서 Delete, 중복키 방지         */
/* PARAMETERS                                                               */
/*   IN p_client_msg_key : 인포뱅크 G/W가 보내온 메시지 키                  */
/*   IN p_date_mo        : mo 발생 시간                                     */
/* REMARK                                                                   */
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmo_tran_delete' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmo_tran_delete
GO

CREATE PROCEDURE sp_em_mmo_tran_delete
    @p_client_msg_key VARCHAR(50)
  , @p_date_mo DATETIME
AS
...
/****************************************************************************/
/* NAME : sp_em_mmo_file_delete                                             */
/* DESC : 수신된 MMS MO Packet을 MMO 첨부파일 테이블에서 Delete, 중복키방지 */
/* PARAMETERS                                                               */
/*   IN p_client_msg_key : 인포뱅크 G/W가 보내온 메시지 키                  */
/*   IN p_date_mo        : mo 발생 시간                                     */
/* REMARK                                                                   */
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_mmo_file_delete' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_mmo_file_delete
GO

CREATE PROCEDURE sp_em_mmo_file_delete
    @p_client_msg_key VARCHAR(50)
  , @p_date_mo DATETIME
AS
    ...
    SELECT @dsql = ' DELETE FROM em_mmo_file_log_' + @v_log_table + ' WHERE mo_key = @pp_client_msg_key '
