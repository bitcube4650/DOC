 ...
 *
 * EMMA가 공통으로 사용하는 SQLServer 2000 Stored Procedure 이다.
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_common_create' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_common_create
GO

CREATE PROCEDURE sp_em_common_create
AS
    ...
    BEGIN
        ...
        ALTER TABLE em_resultcode  ADD PRIMARY KEY(rslt_code)
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_common_banlist' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_common_banlist
GO

CREATE PROCEDURE sp_em_common_banlist
...
IF EXISTS (
    SELECT name
    FROM   sysobjects 
    WHERE  name = 'sp_em_common_checkprivilege' 
    AND    type = 'P' )
    DROP PROCEDURE sp_em_common_checkprivilege
GO

CREATE PROCEDURE sp_em_common_checkprivilege
AS
